...
- The CLI command parser recognizes only `tick`, `start`, `stop`, `api`,
  `ui`, `audit`, `correlate`, and `validate-state`. `init`, `sandbox`,
  `doctor`, `self-update`, and `smoke` exist as composable command functions
  under `cli/commands/` that a caller (Bootstrap, or a future parser
  extension) can invoke directly; they are not reachable through the parsed
  `wake <command>` surface today.
