...
- The deliverable of this plan is a _sound internal model_ — identity, event shapes, and layering that won't need rework — not graph features.
...
| Detection _scanning_ (polling for unregistered marked artifacts)                                                                | Markers are being written from day one (§4), so scanning recovers history whenever it lands                            |
