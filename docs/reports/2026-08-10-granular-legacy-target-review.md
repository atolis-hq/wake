...
| --- | --- | --- |
| `test/adapters/claude-runner.test.ts` | CLI argv, prompt/model/max-turn/tool forwarding, stdout/result parsing, exit/timeout errors | **D** T8 directly proves Claude fresh/resume argv, full-prompt forwarding, structured generic result translation, malformed-output fallback, partial-usage safety, and timeout classification. Automatic fresh fallback after an unavailable session remains deliberately deferred: no authoritative Claude unavailable-session signature is currently available. |
| `test/adapters/codex-runner.test.ts` | Codex argv/model/prompt, parse/failure/timeout | **D** T8 directly proves Codex fresh/resume argv, full-prompt forwarding, structured generic result translation, malformed-output fallback, partial-usage safety, and timeout classification. Automatic fresh fallback after an unavailable session remains deliberately deferred pending an authoritative adapter signature. |
| `test/adapters/cursor-runner.test.ts` | Cursor argv/model/prompt, parse/failure/timeout | **D** T8 directly proves Cursor fresh/resume argv, full-prompt forwarding, structured generic result translation, malformed-output fallback, partial-usage safety, and timeout classification. Automatic fresh fallback after an unavailable session remains deliberately deferred: no authoritative Cursor unavailable-session signature is currently available. |
...
| Resolved with explicit defer | **G-M1:** concrete vendor resume and generic structured-result translation now have direct adapter coverage. | `test-next/integration/execution/process-execution.test.ts` proves Claude/Codex/Cursor fresh and resumed argv shapes, current-prompt forwarding, adapter-private parser translation, raw-output fallback, and partial-usage safety. It does not claim automatic fresh fallback after a failed resume: that behavior is deferred until an adapter has an authoritative unavailable-session signature; timeout, cancellation, and unclassified failure remain non-replayed. |
...
| Low | **G-M8:** the development launcher has no intentional disposition or direct target proof. | Decide whether to preserve it with a target contract test or document its approved removal. |
