...
| T8 | `test-next/integration/execution/process-execution.test.ts` asserts timeout classification; Claude, Codex, and Cursor fresh/resume argv with the full current prompt; their adapter-private structured-output translation to generic output/session/token/cost fields; raw-output fallback; and partial-usage safety. |
...
| T19 | `test-next/integration/surfaces/wake-dev-next.test.ts` proves the public `bin/package.json` `wake-dev` registration invokes the target source-checkout launcher, forwards `--version` to `src-next/main.ts`, preserves the source-checkout-only refusal, and treats Windows shell metacharacters as literal arguments. |
...
| --- | --- | --- |
| `test/adapters/claude-runner.test.ts` | CLI argv, prompt/model/max-turn/tool forwarding, stdout/result parsing, exit/timeout errors | **D** T8 directly proves Claude fresh/resume argv, full-prompt forwarding, structured generic result translation, malformed-output fallback, partial-usage safety, and timeout classification. A failed resumed invocation remains a normal failure: Wake does not detect unavailable sessions or create a fresh session automatically. |
| `test/adapters/codex-runner.test.ts` | Codex argv/model/prompt, parse/failure/timeout | **D** T8 directly proves Codex fresh/resume argv, full-prompt forwarding, structured generic result translation, malformed-output fallback, partial-usage safety, and timeout classification. A failed resumed invocation remains a normal failure: Wake does not detect unavailable sessions or create a fresh session automatically. |
| `test/adapters/cursor-runner.test.ts` | Cursor argv/model/prompt, parse/failure/timeout | **D** T8 directly proves Cursor fresh/resume argv, full-prompt forwarding, structured generic result translation, malformed-output fallback, partial-usage safety, and timeout classification. A failed resumed invocation remains a normal failure: Wake does not detect unavailable sessions or create a fresh session automatically. |
...
| `test/bin/wake-dev.test.ts` | development launcher arguments/errors | **D** T19 proves the preserved public `wake-dev` registration invokes the target `src-next/main.ts` launcher, forwards arguments, refuses a packaged install without its source sibling, and does not evaluate Windows shell metacharacters. |
| `test/cli/audit-command.test.ts` | audit output/exit behaviour | **D** `test-next/integration/surfaces/cli-runtime-commands.test.ts` proves the target `audit` command reads canonical journal facts (not projections), selects every matching stream id across stream kinds, and writes one target JSON line per selected record. |
...
| `test/cli/scaffold-assets.test.ts` | shipped asset creation/customization | **D** T14 and `test-next/integration/surfaces/init.test.ts` prove target initialization writes each supplied asset's exact content and refuses a non-empty root without replacing an operator-customized asset. |
...
| `test/lib/detached-process-logging.test.ts` | detached capture/exit/log path | **D** `test-next/integration/surfaces/process-log.test.ts` proves detached stdout/stderr are drained through close, scrubbed, and durably persisted; sink failure is reported without an unhandled rejected drain promise. |
...
| `test/lib/log-rotation.test.ts` | rotation/bounds/error handling | **D** `test-next/integration/surfaces/process-log.test.ts` proves bounded single-generation rotation before the configured byte limit is exceeded and proves a later write still persists after a transient sink failure. |
...
| Resolved | **G-M1:** concrete vendor resume and generic structured-result translation now have direct adapter coverage. | `test-next/integration/execution/process-execution.test.ts` proves Claude/Codex/Cursor fresh and resumed argv shapes, current-prompt forwarding, adapter-private parser translation, raw-output fallback, and partial-usage safety. A failed resumed invocation is returned through ordinary failure/retry/escalation; Wake does not detect unavailable session IDs or automatically start a fresh session. |
...
| Resolved | **G-M8:** the public development launcher is intentionally preserved at target cutover with direct subprocess proof. | `bin/package.json` maps `wake-dev` to `bin/wake-dev.js`; `test-next/integration/surfaces/wake-dev-next.test.ts` proves target version forwarding, packaged-install refusal, and literal Windows shell-metacharacter forwarding. |
| Resolved | **G-M9:** target audit command is directly proven. | `test-next/integration/surfaces/cli-runtime-commands.test.ts` proves canonical journal reads, cross-stream-id selection, and JSON-line formatting. |
| Resolved | **G-M11:** target initialization preserves operator customization. | `test-next/integration/surfaces/init.test.ts` proves exact supplied asset contents and non-empty-root refusal without overwriting an existing custom asset. |
| Resolved | **G-M13:** target detached logging and bounded rotation are directly proven. | `test-next/integration/surfaces/process-log.test.ts` proves scrubbed durable teeing, rotation, recovery after a write failure, close-drained detached streams, and non-throwing error reporting. |
