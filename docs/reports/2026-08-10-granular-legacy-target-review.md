...
| T14 | `test-next/e2e/scenarios/initialise-root.test.ts:13`, **E2E-OPS-INIT-001**; `doctor-rebuild.test.ts:16,56,86`, **E2E-OPS-001**; `sandbox-routing.test.ts:10`, **E2E-OPS-SANDBOX-001**; `self-update-packaged.test.ts:19`, **E2E-OPS-SELF-UPDATE-001**; `transcript-retention.test.ts:20`, **E2E-OPS-TRANSCRIPT-001**. |
...
| `test/adapters/github-auth.test.ts` | token/auth success/failure | **G-M3** no target test proves GitHub credential acquisition/failure classification. |
| `test/adapters/github-client.test.ts` | request shapes, pagination, errors, comments/labels/PR effects | **G-M4** target asserts neutral effects, not GitHub transport request semantics. |
| `test/adapters/github-client-request-log.test.ts` | request log ordering/redaction | **G-M4**. |
| `test/adapters/github-etag-cache.test.ts` | conditional request, cache hit/miss/invalidations | **G-M4**. |
| `test/adapters/github-issues-work-source.test.ts` | eligibility, labels, dedupe, marker reconciliation, issue lifecycle | **D** T1/T9 and `live-eligibility.test.ts:12` **E2E-LIVE-005** cover intake eligibility/dedupe; **C** marker-family extraction is deferred by `FUTURE-STATE-SYNC-EXTRACTION`; **G-M4** leaves GitHub pagination/ETag transport unproved. |
| `test/adapters/github-pull-request-activity-source.test.ts` | PR observations, comment/review signals, trust/ordering | **D** T3/T12; **G-M4** leaves provider pagination and raw event ordering unproved. |
| `test/adapters/github-pull-request-merge-actor.test.ts` | merge request/method/result failure | **D** T12/T4 prove neutral intent, configured method, confirmation and ambiguity; **G-M4** lacks GitHub request-wire proof. |
| `test/adapters/git-workspace-manager.test.ts` | no/read-only/branch workspace, reuse/conflict, cleanup | **D** `test-next/unit/execution/git-workspace.test.ts:17,44,76,107` and **E2E-EXEC-001**; **G-M5** lacks a target restart/cleanup-crash proof for a real Git workspace. |
...
| `test/adapters/self-update-ledger.test.ts` | idempotent update ledger/interruption | **C** `OPS-SELF-UPDATE` preserves bounded ledger but T14 only proves packaged refusal; **G-M6** no target interruption/recovery assertion. |
...
| `test/adapters/ui-data.test.ts` | work/run/resource presentation and formatting | **D** T13 covers composition, pagination/provenance and board/analytics; **G-M7** legacy formatting permutations are not comparable target contract proof. |
| `test/adapters/ui-server.test.ts` | HTTP/static/UI routing/error responses | **D** T13 production HTTP proof; **G-M7** static asset/routing edge cases lack target parity test. |
...
| `test/cli/main.test.ts` | command parsing, help/version/exit | **G-M10** target surface apps are covered, but no target CLI command/help/exit contract is proven. |
...
| `test/cli/self-update-command.test.ts` | source update/tag/recovery/container rebuild | **D** T14 packaged refusal; **G-M6** source-mode/update-recovery remains unproved. |
| `test/cli/startup-preflight.test.ts` | configuration/auth/Docker diagnostic gate | **D** `doctor-diagnostics.test.ts:34,87,122,161,185`; **G-M3** auth transport detail. |
...
| `test/core/sink-router.test.ts` | route/sequence/error isolation | **D** T4/T5; **G-M12** no target focused multi-sink ordering/isolation test. |
...
| `test/core/tick-runner.workspace-cleanup.test.ts` | cleanup after outcomes/transcript retention | **D** T14 and target workspace unit tests; **G-M5** crash during cleanup. |
| `test/core/workspace-cleanup.test.ts` | cleanup semantics/errors | **D** target workspace units; **G-M5** crash proof. |
...
| `test/lib/format.test.ts` | elapsed/status/format rendering | **G-M7** no target parity catalogue row; presentation changed to domain views. |
...
| `test/scripts/e2e-github-fake.test.ts` | runnable fake e2e script | **D** `github-fake-script.test.ts:5`; **G-M4** does not validate real GitHub script semantics. |
| `test/version.test.ts` | version source/format | **G-M14** no target version contract test. |
...
| --- | --- | --- |
| High | **G-M3/M4:** GitHub auth/client/ETag/request-log behaviour has no target adapter-level proof; externally observable authentication, pagination, conditional reads, redaction, and request idempotency are not covered by neutral E2E effects. | Add integration tests for the target GitHub transport (credential failure, paginated/ETag reads, redacted request log, exact merge/comment request). If GitHub transport is intentionally out of scope, add an explicit catalogue defer/remove row rather than treating generic provider E2E as proof. |
| High | **G-M10:** `cli/main` command/help/exit contract is not tested. | Add CLI black-box tests for command dispatch, invalid input, help/version and operational exit codes, or catalogue an API-only replacement. |
...
| Medium | **G-M2/M6:** sandbox source/packaged command construction, resume/setup/log capture and self-update interruption/source mode are unproved. | Add fake Docker/Git integration tests; retain packaged refusal as one boundary, not whole operational coverage. |
| Medium | **G-M5/M12:** workspace cleanup crash and multi-sink isolation/ordering have no composed failure test. | Extend fault matrix with cleanup-after-allocation and two-sink partial-failure points. |
| Low | **G-M7/M8/M9/M13/M14:** presentation formatting, dev launcher/audit, detached process logging/rotation, and version contract have no intentional disposition or direct proof. | Decide per capability: add small target contract tests, or document removal/defer in the catalogue. |
