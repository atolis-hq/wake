...
| Medium | **G-M5:** workspace cleanup after a process crash has no target restart/reaper proof. | Retain as unresolved: normal failure/retry releases its lease in the same attempt, but crash-orphan cleanup needs a durable workspace ownership/reaper design before it can be claimed. |
