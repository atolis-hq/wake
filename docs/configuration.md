...
| `resourceTransitions` | list | `on.done` only. Each entry is one of `pr.review-accepted`, merged `pr.state-changed`, or failing `pr.checks-changed`; optional `then` inherits the route target. |
