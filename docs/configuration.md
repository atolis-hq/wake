...
| `controlPlane.activationScheduler.mode` | `inline` (default) or `subscriber` | `inline` keeps activation scheduling in the runner pipeline and remains the rollback-safe default. `subscriber` is an explicit opt-in migration: `wake start` supervises one independently checkpointed scheduler subscription, so slow legacy reactor, publication, or delivery work cannot delay activation scheduling. It reconsiders every new durable fact, including capacity release, reconciles on startup and bounded fallback, and uses the same global scheduler lock as one-shot ticks. Subscriber delivery is at least once, so effects must be idempotent from durable identities. Switching back to `inline` is configuration-only; journal facts and subscriber checkpoints remain. |
