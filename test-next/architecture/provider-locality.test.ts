...
describe('provider-locality value scope', () => {
  it('rejects a provider name inside a WorkItem identity argument', async () => {
    const diagnostics = await check({
      'src-next/integrations/github/application/inbound-translator.ts': [
    ...
    expect(messages(diagnostics)).toMatch(
      /src-next\/integrations\/github\/application\/inbound-translator\.ts:3:\d+/,
    );
  });

  it('rejects a provider name inside a Resource stream id', async () => {
    const diagnostics = await check({
      'src-next/integrations/github/application/resource-translator.ts': [
    ...
    expect(messages(diagnostics)).toContain('[provider-locality]');
    expect(messages(diagnostics)).toMatch(
      /src-next\/integrations\/github\/application\/resource-translator\.ts:3:\d+/,
    );
  });
  ...
  it('rejects a provider name inside a work.* or resource.* event type literal', async () => {
    ...
    expect(messages(diagnostics)).toContain('[provider-locality]');
    expect(messages(diagnostics)).toMatch(
      /src-next\/integrations\/github\/infrastructure\/issue-source\.ts:3:\d+/,
    );
  });
...
describe('provider-locality path scope', () => {
  ...
  it('rejects a provider name in a file outside the provider namespace', async () => {
    ...
    expect(diagnostics).toHaveLength(1);
    expect(messages(diagnostics)).toContain('[provider-locality]');
    expect(messages(diagnostics)).toMatch(/src-next\/work\/domain\/leak\.ts:1:\d+/);
  });

  it('permits ExternalResourceKey.adapter and adapterId() values everywhere', async () => {
    const diagnostics = await check({
      'src-next/integrations/github/index.ts': 'export const marker = true;',
      'src-next/resources/domain/fixture.ts': [
  ...
  it('derives providers from the integrations directory automatically, covering a second provider without code changes', async () => {
    const diagnostics = await check({
      'src-next/integrations/contracts/marker.ts': 'export const marker = true;',
      'src-next/integrations/application/marker.ts': 'export const marker = true;',
      'src-next/integrations/delivery/marker.ts': 'export const marker = true;',
