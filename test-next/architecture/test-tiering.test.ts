...
describe('target test tiers', () => {
  it('keeps unit, architecture, integration, and E2E runner ownership disjoint', async () => {
    ...
    const [unit, architecture, integration, e2e, live] = await Promise.all([
      ...
      read('vitest.next.architecture.config.ts'),
    ...
    expect(packageJson.scripts['verify:next']).toContain('npm run test:next:architecture');
    ...
    expect(architecture).toContain("include: ['test-next/architecture/**/*.test.ts']");
