...
describe('GitWorkspaceProvider', () => {
  it('clones a repository into a work-item workspace', async () => {
    ...
    const provider = new GitWorkspaceProvider('/workspaces', { cloneLocator: async () => 'https://github.com/atolis-hq/wake-test.git' }, async (args) => { (commands as string[][]).push([...args]); });
    const lease = await provider.acquire({ mode: 'read-only', workItemId: workId('one'), repositoryResource: { resourceId: resId('workspace-resource'), kind: resourceKind('issue'), externalKey: { adapter: 'github', key: 'atolis-hq/wake-test#1' }, capabilities: [] } });
