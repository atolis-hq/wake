  ...
  async () => {
    ...
    world.registerActivity({
      ...
      handler: {
        async execute() {
          refineAttempts += 1;
          return refineAttempts === 1 ? { kind: 'failed' as const } : { kind: 'done' as const };
        },
      },
    });
    world.registerActivity({
      ...
      handler: {
        async execute() {
          implementAttempts += 1;
          return implementAttempts <= 2 ? { kind: 'failed' as const } : { kind: 'done' as const };
        },
      },
    });
    ...
    expect((await world.viewRuns()).map((run) => run.outcome?.kind)).toEqual([
      'failed',
      'done',
      'failed',
      'failed',
