...
defineScenario(
  ...
  async () => {
    ...
    world.configureWorkflow('default', {
      stages: {
        refine: { activity: 'refine', with: {}, execution: { workspace: 'none' }, on: { done: { then: 'implement' } } },
    ...
    expect(runs.every((run) => run.status === 'succeeded' && run.outcome?.kind === 'done')).toBe(true);
    expect((await world.events('orchestration.stage-entered')).map((event) => event.payload)).toEqual([{ stage: 'refine' }, { stage: 'implement' }]);
