...
it.each(['canonical', 'legacy'] as const)(
  'reads a %s orchestration projection for work detail',
  async (shape) => {
    const workItemId = workId(`detail-${shape}-projection`);
    const workflowId = `workflow-detail-${shape}`;
        ...
        projections: {
          read: async (stream: string, key: string) => {
            ...
            if (stream === 'orchestration' && key === workflowId)
              return { value: storedWorkflowProjection(shape, workflow), lastGlobalPosition: 0 };
...
it.each(['canonical', 'legacy'] as const)(
  'reads a %s orchestration projection for a work transcript',
  async (shape) => {
    const workItemId = workId(`tr-${shape}-projection`);
    const workflowId = `workflow-transcript-${shape}`;
    const workflow = workflowProjectionView(workflowId, workItemId);
    const run = {
      runId: `run-transcript-${shape}`,
      workflowInstanceId: workflowId,
    ...
    const applications = createSurfaceWorkApplications(
      {
        projections: {
          read: async (stream: string, key: string) => {
            ...
            if (stream === 'orchestration' && key === workflowId)
              return { value: storedWorkflowProjection(shape, workflow) };
            if (stream === 'runs-by-workflow-instance' && key === workflowId)
...
function storedWorkflowProjection(shape: 'canonical' | 'legacy', view: WorkflowInstanceView) {
  return shape === 'canonical' ? view : { events: [], view };
}
