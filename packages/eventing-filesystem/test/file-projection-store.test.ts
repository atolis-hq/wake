...
function projectionPath(root: string, namespace: string, key: string): string {
  return join(
    ...
    'projections',
    'projection',
    projectionStorageAddress(namespace),
...
it('stores long projection identities in bounded path components', async () => {
  const root = await mkdtemp(join(tmpdir(), 'wake-projections-'));
  const namespace = `orchestration:${'namespace-'.repeat(80)}`;
  const key = `workflow:${'key-'.repeat(1_000)}`;
  const store = new FileProjectionStore(root);

  await store.write({ namespace, key, lastGlobalPosition: 1, value: { n: 1 } });

  const path = projectionPath(root, namespace, key);
  expect(path.split('/').at(-1)).toHaveLength(48);
  expect(path.split('/').at(-2)).toHaveLength(43);
  await expect(store.read(namespace, key)).resolves.toMatchObject({ value: { n: 1 } });
});
...
it('does not re-read namespace files on a second list() when nothing changed', async () => {
  ...
  // The canonical directory is probed for external atomic writes;
...
it('does not delete processor state when clearing a projection namespace', async () => {
...
it('does not delete corrupt processor state when clearing every projection', async () => {
  ...
  const path = join(
    ...
    'projections',
    'processor-state',
    projectionStorageAddress(deliveryOutcomeConsumer),
    `${projectionStorageAddress('pending-confirmations')}.json`,
  );
  const raw = '{}\n';
