...
const cursorRunnerSettingsSchema = z.object({
  ...
  smokeModel: z.string().default('claude-haiku-4-5'),
  smokePrompt: z.string().default(defaultSmokePrompt),
  timeoutMs: z.number().int().positive().default(30 * 60 * 1000),
  models: modelOverridesSchema.default({ default: 'claude-sonnet-4-6', implement: 'claude-sonnet-4-6' }),
...
export const wakeConfigSchema = z.object({
  ...
  runners: z.record(z.string(), runnerEntrySchema).default({
    ...
    'cursor-standard': { kind: 'cursor', command: 'cursor', model: 'claude-sonnet-4-6', smokeModel: 'claude-haiku-4-5', smokePrompt: defaultSmokePrompt, timeoutMs: 30 * 60 * 1000, models: { default: 'claude-sonnet-4-6', implement: 'claude-sonnet-4-6' } },
  }),
