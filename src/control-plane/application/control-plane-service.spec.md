...
dispatch pause or resume, and the `isDispatchPaused` read that Advancement's
`isDispatchPaused` gate calls before every attempt at recovery,
reconciliation, or selection.

## Ubiquitous language

- **Global dispatch pause command** — the operator-initiated `pauseDispatch`/
  `resumeDispatch` command handled by this component, always recorded as an
  indefinite pause (no `resumeAt` deadline an operator set explicitly; see
  Decisions).

## Responsibilities and boundaries

This component owns accepting a `pauseDispatch`/`resumeDispatch` command, appending the
...
also owns `isDispatchPaused`, the read Advancement's `isDispatchPaused` supplier
...
- `isDispatchPaused` MUST fold every `DispatchPaused`/`DispatchResumed` event on the
...
- `pauseDispatch`/`resumeDispatch` MUST derive a correlation id deterministically from the
...
- Beyond the idempotency-key check, `pauseDispatch` MUST also no-op (append nothing)
  when `isDispatchPaused` already reports `true`, and `resumeDispatch` MUST no-op when
  `isDispatchPaused` already reports `false` — a redundant dispatch-pause/resume command
  never produces a redundant event even under a fresh `idempotencyKey`.
- A `pauseDispatch` command MUST append `DispatchPaused` with a fixed `reason` of
...
- A `resumeDispatch` command MUST append `DispatchResumed` with `resumedAt` set to
...
| --- | --- | --- |
| `idempotencyKey` | string | Caller-supplied key this component's durable dedupe is keyed on, together with the operation name (`pause-dispatch` or `resume-dispatch`). |
...
- `isDispatchPaused` and the Control Plane view projection both fold the same two
